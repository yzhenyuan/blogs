...
- [每个过程的优化](#每个过程的优化)
  - [一、 网络连接优化](#一-网络连接优化)
  - [二、 请求方面优化](#二-请求方面优化)
      - [2.1 合理使用文件的压缩与合并](#21-合理使用文件的压缩与合并)
      - [2.2 压缩图片](#22-压缩图片)
      - [2.3 组件延迟加载](#23-组件延迟加载)
      - [2.4 给 `Cookie` 瘦身](#24-给-cookie-瘦身)
      - [2.5 利用`CDN`提升浏览器资源的加载能力](#25-利用cdn提升浏览器资源的加载能力)
      - [2.6 合理利用缓存策略](#26-合理利用缓存策略)
  - [三、 响应优化](#三-响应优化)
  - [页面加载的核心指标](#页面加载的核心指标)
  - [四、 浏览器首屏渲染优化](#四-浏览器首屏渲染优化)
    - [首屏时间](#首屏时间)
    - [首屏优化方案](#首屏优化方案)
  - [第二部分：性能面板](#第二部分性能面板)
  - [第三部分：Summary（性能摘要）](#第三部分summary性能摘要)
