...
国内一线的大型公司都有自己的产品矩阵，其实并不是JQ不方便，而是为了更好的适配自家的工具链，比如百度有富文本IDE 有RIA体系 有定制化的componen更有类似于百度统计的SDK 、甚至有自己的百度浏览器的内核都会有一些JS的定制API，百度自家的tangram也恰恰是为了整个百度的大体系服务，而不是简简单单的为了兼容IE系和其他浏览器。